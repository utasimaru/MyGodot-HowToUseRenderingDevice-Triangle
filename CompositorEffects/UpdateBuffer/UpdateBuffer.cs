...
[Tool, GlobalClass]
public partial class UpdateBuffer : CompositorEffect
{
	public static List<UpdateBuffer> list = new();
	RenderingDevice rd;
	Rid pipeline;
	Rid shader;
	...
	public void Reverse()
	{
		GD.Print("reverse Triagnle");
		ReadOnlySpan<float> vertices = [0.0f, 0.5f, 0.5f, -0.5f, -0.5f, -0.5f];
		var vertices_position_packed = MemoryMarshal.AsBytes(vertices);
		rd.BufferUpdate(vertex_position_buffer, 0, (uint)vertices_position_packed.Length, vertices_position_packed);
	}
