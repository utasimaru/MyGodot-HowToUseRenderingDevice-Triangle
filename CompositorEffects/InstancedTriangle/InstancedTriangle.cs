...
[Tool, GlobalClass]
public partial class InstancedTriangle : CompositorEffect
{
	RenderingDevice rd;
	Rid pipeline;
	Rid shader;
	...
	public void _initialize_rendering(RenderSceneBuffersRD renderscene_buffers)
	{
		...
		vertex_array = rd.VertexArrayCreate(3, vertex_format, []);
	...
	public override void _RenderCallback(int effectCallbackType, RenderData renderData)
	{
		...
		if (effectCallbackType == ((int)EffectCallbackTypeEnum.PostTransparent)) {
			...
			if (render_scene_buffers != null) {
				...
				rd.DrawListDraw(draw_list, false, 4, 0);
				rd.DrawListEnd();
	...
	const string _default_source_vertex = @"
		...
		void main() {
			...
			float y = gl_VertexIndex / 2 + gl_InstanceIndex;
